// Local transport patch, 2026-09-03. See package PATCHES.md.
import 'dart:async';
...
import '../common/serialized_socket_writer.dart';
...
class _WKSocket {
  ...
  final SerializedSocketWriter _writer;
  _WKSocket._internal(Socket socket)
      : _socket = socket,
        _writer = SerializedSocketWriter(socket);
...
class WKConnectionManager {
  ...
  _sendConnectPacket() async {
    CryptoUtils.init();
    var deviceID = WKIM.shared.options.deviceId ?? await _getDeviceID();
  ...
  _sendPacket(Packet packet) async {
    ...
    if (!isReconnection) {
      ...
      try {
        ...
        if (socket != null && identical(socket, _socket) && !isDisconnection) {
          Logs.debug('Socket write failed; reconnecting: $error');
          _socket = null;
          socket.close();
          setConnectionStatus(WKConnectStatus.fail);
          Future.delayed(Duration(milliseconds: reconnMilliseconds), () {
            if (!isDisconnection && _socket == null) connect();
          });
        }
      }
    }
