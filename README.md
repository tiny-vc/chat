...
App、Android、iOS 和管理平台共用根目录的 `branding.json`。修改产品名称、标语或主色后执行：

```bash
npm run branding:sync
```

带有 `Generated by scripts/sync-branding.mjs` 注释的文件由脚本生成，不要分别手工修改。品牌名称会在下一次构建 APK、IPA 和管理平台静态资源时生效。
...
当前版本是否可以进入真机或正式发布，请按
[第一阶段发布就绪清单](docs/phase-one-release-readiness.md)逐项确认。
...
With `JOBS_ENABLED=true`, an instance whose `INSTANCE_ROLE` is `worker` or `all` runs cleanup and maintenance; `api` instances never start background timers. Single-node deployments use `all`. PostgreSQL advisory locks remain a second guard against overlapping workers. The job retries `DELETE_PENDING` objects, marks stale pending/rejected uploads as deleted after removing their objects, prunes old revoked/expired device sessions, and removes old login throttle rows. Every run is stored in `job_runs` with status and counters. Administrator endpoints can inspect history or trigger the same idempotent job manually.
...
第一阶段的单机与外部基础服务生产拓扑、域名、端口、TLS、启动、升级和验收步骤见
[生产部署文档](docs/production-deployment.md)。生产入口使用
`docker-compose.production.yml`（单机）或 `docker-compose.external-services.yml`
（远程 PostgreSQL、WuKongIM、LiveKit、S3/MinIO）；根目录的
`docker-compose.yml` 仍只用于本地开发。
...
管理员，无需先开放注册。
首次启动或升级 IM/API 后可运行
`sh scripts/verify-wukong-media-webhook-production.sh`，验证 WuKongIM 真实
`msg.notify` 回调能够完成聊天媒体引用确认。
