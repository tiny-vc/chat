...
for (const internalField of ["initiatorSessionId", "targetSessionId"]) {
  if (internalField in call) {
    throw new Error(`Call response leaked internal field: ${internalField}`);
  }
}
const ringingCall = await request(
  `/calls/${call.id}`,
  json("GET", alice.accessToken),
);
if (!["INVITING", "RINGING"].includes(ringingCall.status)) {
  throw new Error(`Unexpected initial call state: ${ringingCall.status}`);
}
...
const endedCall = await request(
  `/calls/${call.id}`,
  json("GET", bob.accessToken),
);
if (endedCall.status !== "ENDED") {
  throw new Error(`Call state did not converge after hangup: ${endedCall.status}`);
}
