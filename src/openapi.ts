...
export function createOpenApiDocument(app: INestApplication): OpenAPIObject {
    ...
    .addBearerAuth(
      {
        ...
        scheme: "bearer",
        bearerFormat: "JWT",
        description: "Business access token",
      },
      "access-token",
    )
    .build();
  ...
  for (const [path, pathItem] of Object.entries(document.paths)) {
    for (const [method, operation] of Object.entries(pathItem ?? {})) {
      ...
      typed.responses ??= {};
      const response = typed.operationId
        ? operationResponseSchemas[typed.operationId]
        : undefined;
      if (response) {
        ...
        const currentResponse = typed.responses[successStatus] as
          { content?: unknown; description?: string } | undefined;
